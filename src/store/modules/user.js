...
import { platformList } from "../../api/game"
...
const user = {
    state: {
        ...
        token: getToken(),
        roles: [],
        browserHeaderTitle: mUtils.getStore('browserHeaderTitle') || '用户管理系统',
        username: "",
        user: {
            email: "265655@qq.com",
            nickname: "sss",
            telphone: "13025255464",
            touziyear: "2",
        },
        gamePlatforms: []
    },
    getters: {
        ...
        gamePlatforms: state => state.gamePlatforms,
    ...
    mutations: {
        ...
        INFORMATION(state, infoForm) {
            state.user = infoForm
        },
        SET_USERNAME(state, username) {
            state.username = username
        },
        GAMEPLATFORMLIST(state, gamePlatforms) {
            state.gamePlatforms = gamePlatforms
        }
    },
    actions: {
        // 登录
        ...
        async Platforms({ commit }) {
            let reslut = await platformList();
            let gamePlatforms = reslut.data.list;
            commit('GAMEPLATFORMLIST', gamePlatforms)
        },
        // 根据token获取用户信息
